...
int manSwitch = 7;  // Compass vs GPS and compass.
...
void loop() {
  ...
  float yVal = 0;
  int nowHeading;
  for (int i = 0;i<50;i++)  {  // 50 readings, 25 milli seconds in between.
...
//  Serial.println(nowHeading);
  ...
  if((time-previousTime > 15000) || (abs(theHeading - previousHeading)> 20)){
    //vibrate when heading changes, or every 15 sec.
    vibrate();
...
void vibrate(){
  ...
  delay (1000);  // Be sure the motors are not spinning before taking a compass reading.  
